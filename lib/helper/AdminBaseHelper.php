...
{
    const INFO_SESSION_FIELD = "ELEMENT_SAVE_INFO";
    const LONG_ACTION_FIELD = "ELEMENT_LONG_ACTION_";
    /**
     * @internal
    ...
    /**
     * @var array
     * Хранит список информационных сообщений
     */
    ...
    public function addInfo($info)
    {
        ...
        if (isset($_SESSION[self::INFO_SESSION_FIELD]) AND !empty($_SESSION[self::INFO_SESSION_FIELD])) {
            $_SESSION[self::INFO_SESSION_FIELD] = array_merge($_SESSION[self::INFO_SESSION_FIELD], $info);
        } else {
            $_SESSION[self::INFO_SESSION_FIELD] = $info;
        }
    }

    public function addLongAction($action_id, $title)
    {
        $_SESSION[self::LONG_ACTION_FIELD][$action_id] = $title;
    }

    public function getLongActions()
    {
        if (!empty($_SESSION[self::LONG_ACTION_FIELD])) {
            return $_SESSION[self::LONG_ACTION_FIELD];
        }

        return [];
    }

    public function checkLongAction($action_id)
    {
        return isset($_SESSION[self::LONG_ACTION_FIELD][$action_id]);
    }

    public function removeLongAction($action_id)
    {
        if (!empty($_SESSION[self::LONG_ACTION_FIELD][$action_id])) {
            unset($_SESSION[self::LONG_ACTION_FIELD][$action_id]);
        }
    }
    ...
    {
        if (isset($_SESSION[self::INFO_SESSION_FIELD]) AND !empty($_SESSION[self::INFO_SESSION_FIELD])) {
            $info = $_SESSION[self::INFO_SESSION_FIELD];
            unset($_SESSION[self::INFO_SESSION_FIELD]);
    ...
    public function getHelperClass($class)
    {
        ...
        foreach ($interfaceSettings as $viewName => $settings) {
            ...
            if (in_array($thirdClass,
                array(AdminSectionListHelper::className(), AdminSectionEditHelper::className()))) {
    ...
    public static function getSectionsEditPageURL($params = array())
    {
        ...
        if (empty(static::$sectionsEditViewName) && class_exists($sectionEditHelperClass)) {
            return $sectionEditHelperClass::getViewURL($sectionEditHelperClass::getViewName(),
                static::$sectionsEditPageUrl, $params);
        } else {
