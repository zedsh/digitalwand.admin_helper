...
abstract class AdminListHelper extends AdminBaseHelper
{
    ...
    public function buildList($sort)
    {
            ...
            while ($data = $res->NavNext(false)) {
                ...
                if ($data['IS_SECTION']) // для разделов своя обработка
                {
                    list($link, $name) = $this->getRow($data,
                        $this->getHelperClass(AdminSectionEditHelper::className()));
        ...
        $long_actions = $this->getLongActions();
        foreach ($long_actions as $long_action_title) {
            \CAdminMessage::ShowNote($long_action_title);
        }

        if(!empty($long_actions)){
        ...
        if (in_array($_GET['mode'], array('list', 'frame'))) {
            ...
            foreach ($info as $info_item) {
                \CAdminMessage::ShowNote($info_item);
            }
